...
test.skip("full adventure cycle: begin → choices → complete → overworld return", async ({
  ...
  // 3. Click a Begin button for an available adventure.
  // 4. Expect NarrativeLog entries to appear as the stream progresses.
  // 5. Respond to a choice prompt; verify next event is rendered.
  // 6. Reach the adventure_complete screen; click "Return to Overworld".
  // 7. Expect the overworld view to be visible again.
});
