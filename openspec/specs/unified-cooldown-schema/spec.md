# Unified Cooldown Schema

## Purpose

Defines the single shared `Cooldown` model used by both adventures and skills. This replaces the former `SkillCooldown` model and the flat `cooldown_ticks` / `cooldown_game_ticks` / `cooldown_days` fields on `AdventureSpec`. All cooldown configurations across the content system use this single model.
