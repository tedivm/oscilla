...
Without a registry, only `known_skills` is returned. The method SHALL accept `registry=None` and degrade gracefully.

#### Scenario: available_skills union from multiple sources

- **WHEN** a player has `known_skills = {"shout"}`, equips a sword with `grants_skills_equipped: ["power-strike"]`, and holds a potion with `grants_skills_held: ["detect-poison"]`
- **THEN** `available_skills(registry)` returns `{"shout", "power-strike", "detect-poison"}`
