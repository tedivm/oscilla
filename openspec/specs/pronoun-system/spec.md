...
Defines how player pronoun sets are stored, serialized, and rendered in narrative content. Provides built-in `they_them`, `she_her`, and `he_him` sets plus extensibility via `CharacterConfig`.
...
- **THEN** the output is `"Praise HE!"` (subject form, uppercased)
