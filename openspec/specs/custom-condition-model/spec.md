# Custom Condition Model

## Purpose

Defines the `CustomCondition` manifest kind and the `CustomConditionRef` condition leaf, which together allow content authors to declare named, reusable conditions that can be referenced by name throughout the content package.
