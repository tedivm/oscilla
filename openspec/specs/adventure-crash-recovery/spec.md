# Spec: Adventure Crash Recovery

## Purpose

Defines how SSE events emitted during an adventure session are persisted so that a browser refresh or network interruption does not lose progress. The `GET /play/current` endpoint is the crash-recovery entry point.
