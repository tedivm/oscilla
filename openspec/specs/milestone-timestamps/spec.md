...
Defines how milestone grant events are recorded with both tick and real-world timestamp data, enabling time-elapsed conditions and audit trails.

## Requirements

### Requirement: `GrantRecord` model captures both time tracks at grant time

A `GrantRecord` Pydantic model SHALL be defined in `oscilla/engine/models/base.py` with two fields:
...
### Requirement: Milestones field stores `GrantRecord` values

The `milestones` field on `CharacterState` SHALL be `Dict[str, GrantRecord]` where the key is the milestone name. `grant_milestone(name)` SHALL create a `GrantRecord` capturing `self.internal_ticks` and `int(time.time())` at call time. If the milestone is already held, the call SHALL be a no-op (the original record is preserved).
...
1. **Old list format** `["name-a", "name-b"]` → `GrantRecord(tick=0, timestamp=0)` per entry. Tick=0 and timestamp=0 are the sentinels meaning "granted before tracking was introduced."
2. **Intermediate int-dict format** `{"name-a": 42}` → `GrantRecord(tick=42, timestamp=0)` per entry. Handles saves from a deployment using the `Dict[str, int]` design before `GrantRecord` was introduced.
3. **Current nested dict format** `{"name-a": {"tick": 42, "timestamp": 1744000000}}` → parsed directly as `GrantRecord`.
...
#### Scenario: has_milestone works with GrantRecord storage

- **WHEN** `state.milestones == {"joined-guild": GrantRecord(tick=42, timestamp=...)}` and `has_milestone("joined-guild")` is called
- **THEN** it returns True

#### Scenario: Old list format is migrated on load

- **WHEN** `from_dict` receives `{"milestones": ["milestone-a", "milestone-b"], ...}`
- **THEN** `state.milestones["milestone-a"] == GrantRecord(tick=0, timestamp=0)` after deserialization
...
- **THEN** `state.milestones["veteran"] == GrantRecord(tick=100, timestamp=0)` after deserialization
- **THEN** a warning is logged per migrated milestone entry

#### Scenario: New nested dict format loads correctly

- **WHEN** `from_dict` receives `{"milestones": {"veteran": {"tick": 100, "timestamp": 1744000000}}, ...}`
- **THEN** `state.milestones["veteran"] == GrantRecord(tick=100, timestamp=1744000000)` after deserialization
