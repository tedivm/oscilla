...
### Requirement: Step-level `requires` condition gates execution

All step types (`narrative`, `combat`, `choice`, `stat_check`, `passive`) SHALL accept an optional `requires: Condition | null` field. When `requires` is present and the condition evaluates to `false` against the current player state, the pipeline SHALL skip the step entirely and continue to the next step as if the step returned `AdventureOutcome.COMPLETED`. When `requires` is absent or `null`, the step always executes normally.

The `requires` check SHALL be performed in `_dispatch()` before any step-type-specific handler is invoked.

#### Scenario: Step with failing requires is silently skipped

- **WHEN** a `NarrativeStep` has `requires: { type: name_equals, value: "Adventurer" }` and the player's name is `"Elara"`
- **THEN** the TUI receives no text output for that step and execution continues with the next step

#### Scenario: Step with passing requires executes normally

- **WHEN** a `NarrativeStep` has `requires: { type: name_equals, value: "Adventurer" }` and the player's name is `"Adventurer"`
- **THEN** the TUI receives the step's text output and any effects are applied

#### Scenario: Step without requires always executes

- **WHEN** a `NarrativeStep` has no `requires` field
- **THEN** the step always executes regardless of player state
