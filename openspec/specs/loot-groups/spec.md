...
Defines the `LootGroup` and `LootEntry` models used by all loot sources (LootTable manifests, enemy loot fields, and inline `item_drop` effects). `LootGroup` is the primary unit of loot resolution; each group draws from its own entry pool independently. `LootEntry` is the leaf node that identifies an item, its draw weight, its grant amount, and an optional gate condition.
