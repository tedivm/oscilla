...
Real-world calendar and time predicates for the condition evaluator. Allows content authors to gate adventures, steps, and requirements on the current real-world date, time, moon phase, zodiac sign, and season — enabling holiday events, time-of-day atmosphere, and seasonal content without any template logic.
...
The `month_is` predicate SHALL evaluate to true when the current month (resolved in the game's configured timezone, or server local time if absent) matches the configured value. The value SHALL accept either an integer (1–12) or a full English month name (case-insensitive). String values SHALL be normalized to integer at parse time; invalid names SHALL raise a validation error.
...
The `day_of_week_is` predicate SHALL evaluate to true when the current day of the week (resolved in the game's configured timezone, or server local time if absent) matches. The value SHALL accept either an integer (0 = Monday … 6 = Sunday) or a full English weekday name (case-insensitive). String values SHALL be normalized to integer at parse time; invalid names SHALL raise a validation error.
...
### Requirement: `date_between` condition predicate

The `date_between` predicate SHALL evaluate to true when today's date (resolved in the game's configured timezone, or server local time if absent) falls within the configured month/day range. Both `start` and `end` are objects with `month` and `day` fields. The range is inclusive on both ends. The `month` field of each boundary SHALL accept an integer (1–12) or a full English month name (same normalization as `month_is`). When `start` comes later in the calendar year than `end`, the range wraps the year boundary. When `start` equals `end`, the predicate SHALL always evaluate to false and log a warning.

#### Scenario: date_between true inside same-year window

- **WHEN** a `date_between: {start: {month: 4, day: 1}, end: {month: 9, day: 30}}` condition is evaluated on June 15
- **THEN** it evaluates to true

#### Scenario: date_between false outside same-year window

- **WHEN** a `date_between: {start: {month: 4, day: 1}, end: {month: 9, day: 30}}` condition is evaluated on November 1
- **THEN** it evaluates to false

#### Scenario: date_between true on start boundary

- **WHEN** a `date_between: {start: {month: 4, day: 1}, end: {month: 9, day: 30}}` condition is evaluated on April 1
- **THEN** it evaluates to true (inclusive boundary)

#### Scenario: date_between true on end boundary

- **WHEN** a `date_between: {start: {month: 4, day: 1}, end: {month: 9, day: 30}}` condition is evaluated on September 30
- **THEN** it evaluates to true (inclusive boundary)

#### Scenario: date_between true at year-wrapping window after start

- **WHEN** a `date_between: {start: {month: 12, day: 1}, end: {month: 1, day: 31}}` condition is evaluated on December 15
- **THEN** it evaluates to true (December is after the start of a wrapping range)

#### Scenario: date_between true at year-wrapping window before end

- **WHEN** a `date_between: {start: {month: 12, day: 1}, end: {month: 1, day: 31}}` condition is evaluated on January 15
- **THEN** it evaluates to true (January is before the end of a wrapping range)

#### Scenario: date_between false in gap of year-wrapping window

- **WHEN** a `date_between: {start: {month: 12, day: 1}, end: {month: 1, day: 31}}` condition is evaluated on June 15
- **THEN** it evaluates to false (June is in the gap between January end and December start)

#### Scenario: date_between zero-duration window always false

- **WHEN** a `date_between: {start: {month: 12, day: 25}, end: {month: 12, day: 25}}` condition is evaluated on any date
- **THEN** it evaluates to false and logs a warning
...
#### Scenario: game.yaml with unrecognized timezone falls back gracefully
