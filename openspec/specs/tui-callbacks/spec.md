# Spec: UICallbacks Protocol

## Purpose

Defines the `UICallbacks` protocol — the single interface through which all adventure engine step handlers interact with the player. Implementations include `RichTUI` (terminal), `WebCallbacks` (SSE), and `MockTUI` (tests).
