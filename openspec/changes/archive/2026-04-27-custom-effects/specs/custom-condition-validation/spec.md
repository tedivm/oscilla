...
Additionally, every `CustomEffectRef` that appears anywhere in the content package — in `Adventure`, `Item`, `Skill`, `Archetype`, `Buff`, or `CustomEffect` manifests — SHALL be checked against `registry.custom_effects`. If the referenced `name` is not registered, the loader SHALL raise a `ContentLoadError` identifying the dangling reference and the manifest in which it was found.
...
Additionally, the dependency graph among `CustomEffect` manifests SHALL be inspected for cycles using the same depth-first search back-edge detection algorithm. If any cycle is found, the loader SHALL raise a `ContentLoadError` naming the cycle participants.
...
#### Scenario: CustomEffect two-node cycle raises ContentLoadError

- **GIVEN** `CustomEffect` `"a"` with body `[{type: custom_effect, name: b}]` and `CustomEffect` `"b"` with body `[{type: custom_effect, name: a}]`
- **WHEN** content is loaded
- **THEN** the loader raises a `ContentLoadError` identifying the `a → b → a` cycle
