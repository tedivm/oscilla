...
The `stat_change` adventure effect SHALL modify a named stat by a signed numeric amount. The `stat` field SHALL reference a stat name declared in `CharacterConfig`. The `amount` field SHALL be a non-zero integer or float. The effect SHALL be validated at content load time: the referenced stat MUST exist in `CharacterConfig` and its declared type MUST be `int` or `float`. A `stat_change` targeting a `bool` or `str` stat SHALL be a content load error.
...
#### Scenario: Setting a null-default str stat

- **WHEN** `stat_set { stat: title, value: "Champion" }` is applied to a player whose `title` is `None`
- **THEN** `player.stats["title"]` equals `"Champion"`
