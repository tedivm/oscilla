...
#### Scenario: Template accesses a derived stat by name

- **WHEN** `{{ player.stats['level'] }}` is rendered and `level` is a derived stat with shadow value `3`
- **THEN** the rendered output contains `"3"`
...
#### Scenario: Template accesses player.level directly is a load error

- **WHEN** a manifest template accesses `player.level` (top-level attribute, not via stats dict)
- **THEN** `load()` raises a `ContentLoadError` during mock render (attribute does not exist on `PlayerContext`)

#### Scenario: Template accesses player.hp directly is a load error

- **WHEN** a manifest template accesses `player.hp`
- **THEN** `load()` raises a `ContentLoadError` during mock render
...
- **WHEN** a manifest template accesses `game.xp_thresholds`
- **THEN** `load()` raises a `ContentLoadError` during mock render (attribute not in `GameContext`)
...
Templates SHALL have access to the following built-in functions without any import statement. Functions from `template-functions-extended` (dice pool, die aliases, display helpers) are additionally available as specified in that capability spec.
...
- **WHEN** `{{ clamp(player.stats["hp"] + 50, 0, player.stats["max_hp"]) }}` is rendered for a player with `hp=80, max_hp=100`
