...
### Requirement: POST /characters/{id}/navigate persists current location

**Reason:** `current_location` no longer exists as a concept. Players do not navigate to a location and wait there—they pick a location and immediately begin an adventure via `POST /play/go`. The navigate endpoint is deleted.

### Requirement: AdventureList begins an adventure via gameSession

**Reason:** `AdventureList.svelte` is deleted. Adventure selection is server-side, performed by `POST /play/go`. The frontend never receives or selects an adventure ref.

### Requirement: OverworldStateRead exposes current_location

**Reason:** There is no concept of a character being "at" a location between adventures. `current_location`, `current_location_name`, `current_region_name`, and `navigation_options` (with `is_current` flag) are all removed.
