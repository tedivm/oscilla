...
def test_login_unverified_blocked_when_verification_required(auth_client: TestClient, monkeypatch: Any) -> None:
    """Login returns 403 for unverified users when require_email_verification=True."""
    auth_client.post(
        "/api/auth/register",
        json={"email": "unverified-block@example.com", "password": "securepass123"},
    )
    monkeypatch.setattr("oscilla.routers.auth.settings.require_email_verification", True)
    response = auth_client.post(
        "/api/auth/login",
        json={"email": "unverified-block@example.com", "password": "securepass123"},
    )
...
def test_login_unverified_allowed_when_verification_not_required(auth_client: TestClient, monkeypatch: Any) -> None:
    """Login succeeds for unverified users when require_email_verification=False."""
    auth_client.post(
        "/api/auth/register",
        json={"email": "unverified-allow@example.com", "password": "securepass123"},
    )
    monkeypatch.setattr("oscilla.routers.auth.settings.require_email_verification", False)
    response = auth_client.post(
        "/api/auth/login",
        json={"email": "unverified-allow@example.com", "password": "securepass123"},
    )
...
def _register_and_login(client: TestClient, email: str, password: str = "securepass123") -> dict[str, Any]:
...
def test_refresh_success(auth_client: TestClient) -> None:
    ...
    response = auth_client.post("/api/auth/refresh", json={"refresh_token": tokens["refresh_token"]})
    assert response.status_code == 200
    body = response.json()
    assert "access_token" in body
    assert "refresh_token" in body
    # Old token must now be invalid
    second = auth_client.post("/api/auth/refresh", json={"refresh_token": tokens["refresh_token"]})
    assert second.status_code == 401


def test_refresh_invalid_token(auth_client: TestClient) -> None:
    """POST /auth/refresh with a bogus token returns 401."""
    response = auth_client.post("/api/auth/refresh", json={"refresh_token": "notavalidtoken"})
    assert response.status_code == 401
...
def test_logout(auth_client: TestClient) -> None:
    ...
    response = auth_client.post("/api/auth/logout", json={"refresh_token": tokens["refresh_token"]})
...
def test_verify_email_success(auth_client: TestClient) -> None:
    ...
    response = auth_client.get(f"/api/auth/verify/{token}")
    assert response.status_code == 204

    # Confirm is_email_verified is now True
    me_after = auth_client.get("/api/auth/me", headers={"Authorization": f"Bearer {tokens['access_token']}"})
...
def test_verify_email_bad_token(auth_client: TestClient) -> None:
    ...
    response = auth_client.get("/api/auth/verify/notavalidtoken")
...
def test_request_password_reset_no_enumeration(auth_client: TestClient) -> None:
    ...
    response = auth_client.post("/api/auth/request-password-reset", json={"email": "ghost@example.com"})
    assert response.status_code == 204
...
def test_password_reset_success(auth_client: TestClient) -> None:
    ...
    new_pw = "correctpass1"
    auth_client.post("/api/auth/register", json={"email": email, "password": old_pw})
    tokens = _register_and_login(auth_client, email, old_pw)
    user_id = UUID(
        auth_client.get("/api/auth/me", headers={"Authorization": f"Bearer {tokens['access_token']}"}).json()["id"]
    )

    token = make_reset_token(user_id=user_id)
    response = auth_client.post(f"/api/auth/password-reset/{token}", json={"new_password": new_pw})
    assert response.status_code == 204

    # Should now be able to log in with the new password
    login_resp = auth_client.post("/api/auth/login", json={"email": email, "password": new_pw})
...
def test_password_reset_bad_token(auth_client: TestClient) -> None:
    ...
    response = auth_client.post("/api/auth/password-reset/notavalidtoken", json={"new_password": "newpassword1"})
    assert response.status_code == 400
...
def test_get_me(auth_client: TestClient) -> None:
    ...
    auth_client.post("/api/auth/register", json={"email": "me@example.com", "password": "securepass123"})
    tokens = _register_and_login(auth_client, "me@example.com")
    response = auth_client.get("/api/auth/me", headers={"Authorization": f"Bearer {tokens['access_token']}"})
    assert response.status_code == 200
...
def test_update_me(auth_client: TestClient) -> None:
    ...
    auth_client.post(
        "/api/auth/register",
        json={"email": "patchme@example.com", "password": "securepass123", "display_name": "Old Name"},
    )
    tokens = _register_and_login(auth_client, "patchme@example.com")
    headers = {"Authorization": f"Bearer {tokens['access_token']}"}

    response = auth_client.patch("/api/auth/me", headers=headers, json={"display_name": "New Name"})
    assert response.status_code == 200
...
def test_request_password_reset_existing_email_sends_email(auth_client: TestClient) -> None:
    ...
    response = auth_client.post("/api/auth/request-password-reset", json={"email": email})
