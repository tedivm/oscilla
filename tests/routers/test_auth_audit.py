...
async def test_successful_login_creates_login_success(auth_client: TestClient, db_session: AsyncSession) -> None:
    ...
    auth_client.post("/auth/register", json={"email": email, "password": "securepass123"})
    resp = auth_client.post("/auth/login", json={"email": email, "password": "securepass123"})
    assert resp.status_code == 200
    me_resp = auth_client.get("/auth/me", headers={"Authorization": f"Bearer {resp.json()['access_token']}"})
...
async def test_failed_login_creates_login_failure(auth_client: TestClient, db_session: AsyncSession) -> None:
    ...
    auth_client.post("/auth/register", json={"email": email, "password": "securepass123"})
    resp = auth_client.post("/auth/login", json={"email": email, "password": "wrongpassword"})
...
async def test_logout_creates_logout_event(auth_client: TestClient, db_session: AsyncSession) -> None:
    ...
    auth_client.post("/auth/register", json={"email": email, "password": "securepass123"})
    login_resp = auth_client.post("/auth/login", json={"email": email, "password": "securepass123"})
...
async def test_email_verify_creates_verify_event(auth_client: TestClient, db_session: AsyncSession) -> None:
    ...
    me_resp = auth_client.get("/auth/me", headers={"Authorization": f"Bearer {access_token}"})
    user_id = UUID(me_resp.json()["id"])

    token = make_verify_token(user_id=user_id)
    verify_resp = auth_client.get(f"/auth/verify/{token}")
