...
pytest --cov=./oscilla --cov-report=term-missing tests

# Generate HTML coverage report
pytest --cov=./oscilla --cov-report=html tests
# Open htmlcov/index.html in your browser

# Show coverage for specific file
pytest --cov=./oscilla/services/cache.py tests/services/test_cache.py
```
...
def test_version_command():
    ...
    assert "oscilla" in result.stdout
```
...
```python
@pytest.mark.asyncio
@patch('oscilla.services.cache.get_cached')
...
async def test_cache_set_and_get():
    ...
    from oscilla.services.cache import set_cached, get_cached
...
def test_settings_load():
    ...
    assert settings.project_name == "oscilla"
...
async def test_with_clean_cache():
    ...
    from oscilla.services.cache import clear_cache
...
pytest --cov=./oscilla --cov-fail-under=80 tests
```
