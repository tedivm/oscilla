...
The CLI application is defined in `oscilla/cli.py` and automatically configured as an entry point in `pyproject.toml`:
...
After installation, the CLI is available as the `oscilla` command.
...
```bash
oscilla my-command --option value argument
```
...
Add a basic command to `oscilla/cli.py`:
...
```bash
oscilla process input.txt --output output.txt --verbose
oscilla process input.txt -o output.txt -v
```
...
```bash
oscilla export --count 50 --format yaml --output data.yaml
```
...
```python
from oscilla.services.db import get_engine
...
oscilla db migrate
oscilla db seed
oscilla user create john john@example.com
oscilla user list
```
...
```python
from oscilla.commands import database, users
...
def test_version_command_runs():
    ...
    assert "oscilla" in result.stdout.lower()
